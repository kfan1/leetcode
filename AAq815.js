var numBusesToDestination = function (routes, source, target) {
  let buses = 0;
  let stopsWeBeenToObj = {};
  stopsWeBeenToObj[source] = 'x';
  let beenToAllStops = false;
  let routeObj = {};
  for (let i = 0; i < routes.length; i++) {
    routeObj[i] = new Set(routes[i]);
  }

  while (stopsWeBeenToObj[target] === undefined && !beenToAllStops) {
    buses++;
    beenToAllStops = true;
    for (let key in stopsWeBeenToObj) {
      for (let routeKey in routeObj) {
        if (routeObj[routeKey].has(Number(key))) {
          let route = Array.from(routeObj[routeKey]);
          for (let k = 0; k < route.length; k++) {
            if (stopsWeBeenToObj[route[k]] === undefined) {
              beenToAllStops = false;
              stopsWeBeenToObj[route[k]] = 'x';
            }
          }
        }
      }
  ...
  return stopsWeBeenToObj[target] ? buses : -1;
};
...
//       for (let routeIndex of stopToRoutesMap.get(currentStop)) {
//         if (visitedRoutes.has(routeIndex)) continue;

//         visitedRoutes.add(routeIndex);

//         for (let j = 0; j < routes[routeIndex].length; j++) {
//           const nextStop = routes[routeIndex][j];
...
console.log(numBusesToDestination(routes2, source2, target2));

console.log(numBusesToDestination(route3, source3, target3));
